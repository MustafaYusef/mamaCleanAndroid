package com.croczi.mamaclean.ui.Main.myPackage
...
class MyPackage_fragment : Fragment(),MainLesener , PermissionListener {
    ...
    override fun OnSuccessGetMyPackages(it1: myPackages) {
        ...
        mainPackageList?.layoutManager = GridLayoutManager(
            context!!,
            calculateNoOfColumns(context!!, 180f)
        )
        mainPackageList?.adapter=myPackagesAdapter(context!!,it1)
        println("my packages     "+it1)
        mainScroll?.setOnRefreshListener {
            viewModel?.getMyPackages(MainActivity.cacheObj.token,currentLocation!!.latitude,
    ...
    companion object {
        fun newInstance() = MyPackage_fragment()
    }
        ...
        viewModel?.Auth = this
            ...
            mainScroll?.setOnRefreshListener {
                if(currentLocation!=null){
                    ...
                    viewModel?.getMyPackages(MainActivity.cacheObj.token, currentLocation!!.longitude
