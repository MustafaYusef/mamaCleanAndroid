...
class Login : Fragment(),AuthLesener {
    ...
    override fun OnSuccesSignIn(response: signUpResponse) {
        ...
        if(!response.data.isActive){
          ...
          view?.findNavController()?.navigate(R.id.activateUser,bundel)
            ...
            view?.findNavController()?.navigate(R.id.categoreFragment)
    ...
    override fun onFailer(message: String) {
        logInBt?.visibility=View.VISIBLE
        context?.toast(context?.resources!!.getString(R.string.invaledPhoneOrPass))
    }
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        forgetPass?.setOnClickListener {
            view?.findNavController()?.navigate(R.id.sendCode)
        }
