...
class LoginViewModel(val repostary: AuthRepostary) : ViewModel() {
    ...
    fun SignIn(name:String,phone:String,password:String,cityId:Int,playerId:String){
        Auth?.OnStart()
        corurtins.main {
            try {
                val CarsDetailsResponse=repostary.SignIn(name,phone,password,cityId,playerId)
